...
def calcCornerHeuristic(position, bitmaskCorner, corners):
    if bitmaskCorner == (1 << 4) - 1:
        return 0
    cur = 99999999
    for i in range(0, 4):
        if bitmaskCorner & (1 << i) == 0:
            corner = corners[i]
            cost = abs(position[0] - corner[0]) + abs(position[1] - corner[1])
            cur = min(cur, calcCornerHeuristic(corners[i], bitmaskCorner | (1 << i), corners) + cost)
    return cur
...
def cornersHeuristic(state, problem):
    ...
    return calcCornerHeuristic(state[0], state[1], corners)
